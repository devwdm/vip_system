...
func AddSession(sess *model.Session) error {
	//写sql语句
	sqlStr := "insert into sessions(SessionID,UserName,AdminID) values(?,?,?)"
	//执行sql
	_, err := utils.Db.Exec(sqlStr, sess.SessionID, sess.UserName, sess.AdminID)
	if err != nil {
		return err
	}
	return nil
...
func GetSession(sessID string) (*model.Session, error) {
	//写sql语句
	sqlStr := "select SessionID,UserName,AdminID from sessions where  SessionID= ?"
	//预编译
	...
	row.Scan(&sess.SessionID, &sess.UserName, &sess.AdminID)
	return sess, nil
}
